...
(async function() {
  ...
  const nodes = data.nodes.map(n => {
    ...
    return {
      ...
      // Hide the default label below the dot — Obsidian only labels on hover.
      // We show the label as the hover title instead.
      font: { color: theme.text, face: 'Noto Sans SC, sans-serif', size: 0 },
    };
  });
  ...
  const edges = data.edges.map(e => ({
    ...
    // Obsidian-style: NO ARROWS. Edges are undirected lines showing
    // "these two pages reference each other", not directional flow.
    arrows: '',
    // No hover title — Obsidian doesn't label edges either. The edge
    // click popup carries all the source/target context.
    title: '',
