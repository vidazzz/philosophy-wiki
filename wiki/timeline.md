...
(async function() {
  ...
  const timeline = new vis.Graph2d(container, items, {
    ...
    // Graph2d still accepts `groups` for named groupings; with vertical
    // orientation, group labels render along the value (X) axis so
    // each column is self-labelled (时期 / 学派 / 哲学家).
    ...
    // so the bars don't kiss the chart border.
    dataAxis: {
      orientation: 'left',
      visible: true,
